...
def model_pred_group(model, infile, outfile,dataset,seed):
    os.system('python ann_test.py -tt=%s -model=%s -output=%s -set=%s -ds=%s' % (infile, model, outfile,dataset,seed))
...
def get_model_and_time(train_file, outdir,dataset,num,seed):
    ...
    os.system('python ann_train.py -data=%s -model=%s -set=%s -num=%s -ds=%s' % (train_file, model_file,dataset,num,seed))
...
if __name__ == "__main__":
    ...
    print("train time",time_for_train)
