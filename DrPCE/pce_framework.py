...
def model_pred_group(model, infile, outfile,poly,dataset,seed):
    os.system('python tpce.py -tt=%s -model=%s -output=%s -poly=%s -dataset=%s -o=%s' % (infile, model, outfile,poly,dataset,seed))
    if not os.path.exists(outfile):
        raise FileNotFoundError(outfile)


# 1. train model
def get_model_and_time(train_file, outdir,poly,dataset,trainset,seed):
    train_file = os.path.abspath(train_file)
    model_file = os.path.join(outdir, f'modelx{poly}.pt')
    os.system('python pce.py -tt=%s -model=%s -poly=%s -set=%s -train=%s -ds=%s' % (train_file, model_file,poly,dataset,trainset,seed))
...
# 2. test model
def test_model(model, test_file, output_dir,poly,dataset,seed):
    ...
    for ln in contents:
        ...
        test_one_file(model, fn, pnr, output_dir,poly,dataset,seed)


def test_one_file(model, ftest, parameter, output_dir,poly,dataset,seed):
    ...
    with open(pred_file, 'w') as f:
        f.write('\n'.join(p_content))
    model_pred_group(model, pred_file, pred_rfile,poly,dataset,seed)
...
def calculate_gummel_test(model, config_file, output_dir,poly,dataset):
    ...
    for idx1, params in enumerate(p):
        ...
        for idx2, vg in enumerate(vg_all):
            ...
            with open(pred_file, 'w') as f:
                f.write('\n'.join(result))
            model_pred_group(model, pred_file, predr_file,poly,dataset,seed)
...
def orign_calculate_gummel_test( config_file, output_dir):
    orign_max_suffix = '385.txt'
    orign_min_suffix = '1.txt'
    max_file = os.path.join(output_dir, '%s' % (orign_max_suffix))
    min_file = os.path.join(output_dir, '%s' % (orign_min_suffix))
    plot_orign_deriv(max_file, "upper", 0, output_dir)
    plot_orign_deriv(max_file, "upper", 4, output_dir)
    plot_orign_deriv(max_file, "upper", 7, output_dir)
    plot_orign_deriv(min_file, "lower", 0, output_dir)
    plot_orign_deriv(min_file, "lower", 4, output_dir)
    plot_orign_deriv(min_file, "lower", 7, output_dir)


def plot_orign_deriv(file,title,vgth,output_dir):
    ...
    vg0ids = []
    for i in vg.index:
        if (vg[i] == float(vgth)/10):
            vg0vd.append(vd[i])
            vg0ids.append(ids[i])
        # datas = {'Ids_Vx': np.c_[vxs, ids]}
    plot_figure('Parmeters(%s),Vg(%s),I' % (title, str(vgth)), 'Vx', 'I', '', {'Ids_Vx': np.c_[vg0vd, vg0ids]},
                output_dir)
    derivative = diff_forward(vg0vd, vg0ids)
    x1 = np.array(vg0vd[1:])
    x2 = np.array(vg0vd[:-1])
    vxs1 = (x1 + x2) / 2
    plot_figure('Parmeters(%s),Vg(%s),I\'' % (title,  str(vgth)), 'Vx', 'I\'', '',
                {'dIds/dVx_Vx': np.c_[vxs1, derivative]}, output_dir)
    vxs2 = (vxs1[1:] + vxs1[:-1]) / 2
    plot_figure('Parmeters(%s),Vg(%s),I\'\'' % (title,  str(vgth)), 'Vx', 'I\'\'', '',
                {'d^2Ids/d^2Vx_Vx': np.c_[vxs2, diff_forward(vxs1, derivative)]}, output_dir)
...
if __name__ == "__main__":
    ...
    model = get_model_and_time(train_file, output_dir1,poly,dataset,trainset,seed)
    time_for_train = time.time() - start_train
    print(time_for_train)
    start_pred = time.time()
    test_model(model, test_file, output_dir1,poly,dataset,seed)
    ...
    calculate_gummel_test(model, config_file, output_dir1,poly,dataset)
